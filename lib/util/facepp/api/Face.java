package com.facerecognition.api;
...
public class Face {
	...
	private void init(JSONObject json) throws JSONException {
		...
		if (!json.isNull("position")) {
			// 人脸检测的时候有smiling,人脸识别的时候没有，人脸识别的json和人脸检测的数据是不一样的
			if (!json.isNull("attribute")&&!json.getJSONObject("attribute").isNull("smiling")) {
				smiling = new Smiling();
				smiling.smiling = json.getJSONObject("attribute")
		...
		if (!json.isNull("attribute")) {
			...
			// json3 = json2.getJSONObject("smiling");
			// smiling.smiling = json3.getString("value");
		}
	}
	...
	public String toString() {
		return "[face]={id=" + id + "; gender=" + gender.generType.name() + "("
				+ gender.confidence + "); age=" + age.value + "(" + age.range
				+ "); Race=" + race.raceType.name() + "(" + race.confidence
				+ ")";
	}
}
